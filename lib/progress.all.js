...
(function () {
    ...
    progress.data.JSDO = function JSDO(resNameOrParmObj, serviceName) {
        ...
        // used to tell the SmartComponent Library backend which tables
        // in addition to the tableRef table should be populated form
        // the backend during read requests.
        this.viewTables = "";
...
(function () {
    ...
    function initializeJSDO(transport, options) {
        ...
        // Mike Fechner, Consultingwerk Ltd. 16.03.2016
        // Pass viewTables parameter to JSDO for adding this to the JFP
        if (options.viewTables != undefined) {
            jsdo.viewTables = options.viewTables;
        }
    ...
    JSDOTransport = kendo.data.RemoteTransport.extend({
        init: function (options) {
            ...
            // Mike Fechner, 19.06.2016 
            // Callback for after the count method was invoked
            this.afterCountCallback = null;
                    ...
                    if (success) {
                        ...
                        if (options.data && options.data.take) {
                            if (!transport.readLocal && 
                                ...
                                // Minor fix suggested by Edsel Garcia/Progress Software 
                                ...
                                if (options.data.skip === 0 && 
                                    ...
                                    // Mike Fechner, 19.06.2016 
                                    // Callback for after count invoke
                                    if (typeof(transport.afterCountCallback) === "function")
                                    {
                                        transport.afterCountCallback (data.data.length);
                                    }
                                ...
                                promise.done(function(jsdo, success, request){
                                    ...
                                    try {
                                        if (typeof(request.response) === "object" && 
                                            ...
                                            if (typeof(transport.afterCountCallback) === "function")
                                            {
                                                transport.afterCountCallback (total);
                                            }
